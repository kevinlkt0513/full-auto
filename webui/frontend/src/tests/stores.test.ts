...
describe("wizardStore", () => {
  ...
  it("isStepHidden gopay: keep step 7 (slot reused), hide 8 + 14", () => {
    const s = useWizardStore();
    s.setAnswer("payment", { method: "gopay" });
    expect(s.isStepHidden(7)).toBe(false);
    expect(s.isStepHidden(8)).toBe(true);
    expect(s.isStepHidden(14)).toBe(true);
  });

  it("isStepHidden paypal: hide 8 + 14", () => {
    const s = useWizardStore();
  ...
  it("isStepHidden card: hide 7", () => {
    const s = useWizardStore();
    s.setAnswer("payment", { method: "card" });
    expect(s.isStepHidden(7)).toBe(true);
    expect(s.isStepHidden(8)).toBe(false);
    expect(s.isStepHidden(14)).toBe(false);
  });
